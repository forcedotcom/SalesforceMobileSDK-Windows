...
(function () {
    ...
    function handlesyncupdate(sync) {
        var manager = Salesforce.SDK.Hybrid.SmartSync;
        var syncmanager = manager.SyncManager();

        if (manager.Models.SyncStatusTypes.done != sync.Status) return;
        switch (sync.SyncType) {
            case manager.Models.SyncTypes.syncUp:
                //RemoveDeleted();
                //ResetUpdated();
                //SyncDownContacts();
                break;
            case manager.Models.SyncTypes.syncDown:
